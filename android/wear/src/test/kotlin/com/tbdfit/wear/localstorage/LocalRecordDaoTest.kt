...
// Runs against Robolectric's real (non-fake) SQLite implementation on the JVM — this proves
// durable-to-disk commits survive a fresh object graph reading the same file, which is the
// strongest automated proxy for process death available without a real Wear OS
// emulator/device (this sandbox has no hardware virtualization). It is NOT a substitute for the
// real on-device process-death check — see the manual verification steps in the project report.
...
class LocalRecordDaoTest {
    ...
    @Test
    fun newRecordIsPendingReplicationUntilMarkedPhoneSynced() = runTest {
        val db = AppDatabase.build(context, dbName)
        val record = LocalRecordEntity(
            id = UUID.randomUUID().toString(),
        ...
        db.localRecordDao().markPhoneSynced(record.id, System.currentTimeMillis())

        assertTrue(db.localRecordDao().getPendingReplication().isEmpty())
        assertNotNull(db.localRecordDao().getAll().first().first { it.id == record.id }.phoneSyncedAt)
        db.close()
    }
}
